## python custom_rm_processor.py [-h] bird_pos.txt bird_seq.fasta
...
parser = argparse.ArgumentParser(
	...
	epilog="Author: Andrew Mason; Release: 06/10/15; Contact: andrew.mason@roslin.ed.ac.uk")
...
for outfile in os.listdir("."):
	for ext in (".cat", ".cat.gz", ".masked", ".tbl"):
		try:
			...
			pass
#print((usr_args.pos_file).split("_")[0] + " - RepeatMasker complete")

# Call the fragment_remover.py protocol
subprocess.call(("python /groups2/avian_genomes/chicken_erv/04_avian_lineage_LTR_retrotransposon_full_analysis/00_pipeline/002_rm_fragment_remover.py -p " + seq_edit_out + ".out " + pos_edit_out + " &> /dev/null"), shell=True)
#print((usr_args.pos_file).split("_")[0] + " - fragment_remover protocol complete")
...
val_out = ((usr_args.seq_file).split("_")[0]) + "_validated_pos.txt"
...
subprocess.call(("python /groups2/avian_genomes/chicken_erv/04_avian_lineage_LTR_retrotransposon_full_analysis/00_pipeline/003_pos_merger.py --prefix " + ((usr_args.seq_file).split("_")[0]) + " " + val_out) + " &> /dev/null", shell=True)
subprocess.call(("mv " + ((usr_args.seq_file).split("_")[0]) + "_merged_positions.txt " + val_out + " ;chmod u+x " + val_out), shell=True)
	...
	if outfile.startswith((sys.argv[1]).split("_")[0]):
		for ext in ("edit.txt", "edit.fasta", "edit.fasta.out", "annotations.txt", ".tmp"):
			if outfile.endswith(ext):
				os.remove(outfile)
#print((sys.argv[1]).split("_")[0] + " - reverse replacements complete")
