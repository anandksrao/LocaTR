### python rm_fragment_remover.py [-h] [--ori_seq_file ORI_SEQ_FILE] [-p] <rm_out> <ori_pos> <ref_genome> 
...
parser = argparse.ArgumentParser(
	...
	epilog="Author: Andrew Mason; Release: 19/03/15; Contact: andrew.mason@roslin.ed.ac.uk")
...
#parser.add_argument("ref_genome", help="Specify reference genome file for sequence extract")
...
#print pos number and length info
#subprocess.call(("echo; echo \"Total positions: \"; wc -l " + p + "_fragments_removed.txt | awk \'{print $1}\'"), shell=True)
#subprocess.call(("echo \"Sequence length: \"; awk '{sum+=($3-$2)}END{print sum / 1000000, \"Mb\"}' " + p + "_fragments_removed.txt; echo \"Extracting sequences.\""), shell=True)

#extract sequences
#ref_dict = static_functions.seq_dict_creator(usr_args.ref_genome)
#pos_list = static_functions.list_initial_formatter(p + "fragments_removed.txt")
#static_functions.seq_extract(p + "fragments_removed_seq.fasta", pos_list, ref_dict, "N")
#subprocess.call("mv original_pos_no_non-ltr_annotations.txt " + p + "_original_pos_no_non-ltr_annotations.txt", shell=True)
#subprocess.call("mv hits_with_non_ltr_fragments.txt " + p + "_hits_with_non_ltr_fragments.txt", shell=True)
subprocess.call("rm " + p + "fragments_removed* " + p + "hits*", shell=True)

#print("Process complete for " + p[:-1] + ".\n")
